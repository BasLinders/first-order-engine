...
class VizEngine:
    ...
    def get_srm_viz_data(
        visitor_counts: List[int], expected_counts: List[float]
    ) -> Dict[str, Any]:
    ...
    def get_z_distribution_coords(
        mean: float, std_error: float, n_points: int = 100
    ) -> Dict[str, Any]:
    ...
    def get_power_curve_coords(
        ...
        z_alpha = norm.ppf(
            1 - alpha / (2 if alternative == AlternativeHypothesis.TWO_SIDED else 1)
        )
    ...
    def get_bayesian_density_coords(
        alpha: float, beta_param: float, n_points: int = 100
    ) -> Dict[str, Any]:
    ...
    def get_bayesian_winner_status(
        prob_variant_best: float, prob_control_best: float, threshold: float
    ) -> Dict[str, str]:
    ...
    def get_sequential_chart_data(trajectory_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if pd.api.types.is_datetime64_any_dtype(df_safe["measurement_date"]):
            df_safe["measurement_date"] = df_safe["measurement_date"].dt.strftime(
                "%Y-%m-%d"
            )
    ...
    def get_interaction_forest_plot(model) -> Dict[str, Any]:
        ...
        for name, coef, low, high in zip(
            params.index, params.values, conf[0].values, conf[1].values
        ):
    ...
    @staticmethod
    def get_interaction_point_coords(
        df: pd.DataFrame, kpi: str, segment_column: str
    ) -> Dict[str, Any]:
        """Calculates the mean KPI for every 'Variant X Segment' pair."""
        means = (
            df.groupby(["experience_variant_label", segment_column], observed=True)[kpi]
            .mean()
            .reset_index()
        )
