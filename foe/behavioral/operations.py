...
class BehavioralEngine:
    ...
    def detect_outliers_mask(
        ...
        if (
            ...
            or "experience_variant_label" not in df.columns
        ):
            return [False] * len(df)

        if len(df) > large_file_limit:
            # Fully vectorized IQR Method (Fastest)
            q1 = df.groupby("experience_variant_label")[kpi].transform(
                lambda x: x.quantile(0.25)
            )
            q3 = df.groupby("experience_variant_label")[kpi].transform(
    ...
    def apply_transformations(
        df: pd.DataFrame, kpi: str, method: str, params: Dict[str, Any]
        ...
        if method == "Winsorizing":
            if params.get("logic") == "Percentile":
                ...
                lower, upper = np.percentile(
                    df_processed[kpi].dropna(), [low_p, high_p]
                )
    ...
    def run_welch_inference(
        self, df: pd.DataFrame, kpi: str, control_label: str, alpha: float = 0.05
    ) -> Dict[str, Any]:
        """
        ...
        if len(variants) != 2:
            return {
                "error": "Welch's t-test engine requires exactly 2 variants in the dataset."
            }

        if control_label not in variants:
            return {
                "error": f"Control label '{control_label}' not found in dataset variants."
            }

        # Determine the challenger label dynamically
        challenger_label = str(
            variants[1] if variants[0] == control_label else variants[0]
        )

        # Extract groups, dropping NaNs to prevent scipy calculation errors
        group_control = df[df["experience_variant_label"] == control_label][
            kpi
        ].dropna()
        group_challenger = df[df["experience_variant_label"] == challenger_label][
            kpi
        ].dropna()

        # equal_var=False triggers the Welch's Satterthwaite effective degrees of freedom
        t_stat, p_val = stats.ttest_ind(
            group_challenger, group_control, equal_var=False
        )
        ...
        lift = (
            (mean_challenger - mean_control) / mean_control
            if mean_control != 0
            else 0.0
        )
        ...
        return {
            ...
            "group_challenger": {
                ...
                "mean": mean_challenger,
                "n": len(group_challenger),
            },
