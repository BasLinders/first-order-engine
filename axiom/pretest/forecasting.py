...
class ForecastingEngine:
    ...
    def run_seasonal_forecast(
        ...
        """
        if df.empty or 'ds' not in df.columns or 'visitors' not in df.columns or 'conversions' not in df.columns:
            return {"forecast": [], "conclusion": "Invalid input data: Missing required columns (ds, visitors, conversions)."}

        # Prophet typically needs at least a few weeks of data to detect seasonality
        if len(df) < 14:
            return {"forecast": [], "conclusion": "Insufficient historical data. At least 14 days are recommended for seasonal forecasting."}
        ...
        for col in ['visitors', 'conversions']:
            ...
            m = Prophet(yearly_seasonality=True, weekly_seasonality=True, daily_seasonality=False, interval_width=interval)
            ...
            future_forecast = forecast[forecast['ds'] > max_date][['ds', 'yhat', 'yhat_lower', 'yhat_upper']]
